-- Give the planner a single tenant key before it touches high-volume tables.
-- RLS remains enabled and enforced; this helper only resolves the caller's own
-- membership and deliberately errors if the one-user/one-brand invariant is broken.

create function private.current_brand_id()
returns uuid
language sql
stable
security definer
...
revoke all on function private.current_brand_id() from public, anon;
...
create or replace function public.portal_signup_series()
returns table (
  signup_date date,
  signup_count bigint,
  window_end date
...
create or replace function public.portal_contacts_page(
...
create or replace function public.portal_contacts_count(search_text text default null)
returns bigint
language sql
stable
security invoker
